...
*/
    ...
    ofstream outFile;
    outFile.open("ene.csv",ios::out);
    outFile<< "energy"<<","<<"location"<<","<<"error"<<"\n";

    TFile *root_file;
    TH1D *h;
    TString  file;
    string path="/home/rocky/software/GitGate_projectDemo/ene_iso";
    ...
    string a[6]={"165","200","250","300","350","400"};
    Double_t x[48];
    ...
    for(Int_t i=0;i<6;i++)//6
    {
        for(Int_t j=0;j<16;j++)//16
        {
            ...
            {
                file.Form("%s/%s-202107121601/output%s/%d_receptor%d.root",\
