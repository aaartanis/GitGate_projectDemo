...
*/
    ...
    ofstream outFile;
    outFile.open("RF.csv",ios::out);
    outFile<< "width"<<","<<"location"<<","<<"error"<<"\n";

    TFile *root_file;
    TH1D *h;
    TString  file;
    string path="/home/rocky/software/GitGate_projectDemo/RF_iso";
    ...
    for(Int_t i=0;i<6;i++)//6
    {
        for(Int_t j=0;j<16;j++)//16
        {
            ...
            {
                file.Form("%s/%s-202107140851/output%s/%d_receptor%d.root",\
