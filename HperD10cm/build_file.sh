...
# 如果有其他变量的话增加循环的层数即可，增加
	for energy in 165 200 250 300 350 400;do
		mkdir "$energy-$sobp-$time2"
		cd "$energy-$sobp-$time2"
		cp -r ../base/mac ./
		cp -r ../base/data ./
		#mkdir output
		cp -r ../base/output ./
		...
		sed -i "/\/control\/execute/s/400/$energy/" mac/HIMM_room2.mac
		#改变HIMM_rom2.mac的ridge filter,替代400，更换不同能量对应的ridge filter,sed 后有变量时使用双引号

		sed -i "/\/control\/execute/s/6cm/$sobp"cm"/" mac/HIMM_room2.mac
		#改变HIMM_rom2.mac的ridge filter,替代6，更换不同展宽宽度对应的ridge filter,sed 后有变量时使用双引号//uyaoxiugai 
